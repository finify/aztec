...
if($deposit_type == 0){
    if (isset($_POST['submit']))
    {
      ...
      if($reffereeid === 0){
          ...
          if($row11){
            ...
            $newrefuserbalance = $refuserbalance + $refearning;
              $created = date("Y/m/d");
                ...
                $message1 .= "<h3> Hi {$refuserfirstname} {$refuserlastname}</h3>";
                $message1 .= "<h3> You earned a referral bonus of  $". $refearning ."</h3>";
        ...
        $message .= "<h3> Hi {$firstname} {$lastname}</h3>";
        $message .= "<h3> Your Deposit of  $". $usdamount ." have been Approved and your wallet updated</h3>";
  ...
  if (isset($_POST['submit']))
  {
    ...
    if($deposit_to == 1){ //send to withdraw balance
      ...
      if($reffereeid === 0){
        ...
        $message .= "<img src='{$sitelogo}' alt='{$sitename}' style='width:200px; display:block; margin:auto;' >";
        $message .= "<h3> Hi {$firstname} {$lastname}</h3>";
        $message .= "<h3> Your Deposit of  $". $usdamount ." have been Approved and your Investment have started counting.</h3>";
        $message .= '<p>login to your account dashboard to start Investing</p>';
        $message .= '</div>';
        $message .= "</body></html>";
