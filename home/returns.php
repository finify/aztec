            ...
            if($plan_roi_type == 'daily'){ //check for roi type if daily or others
                ...
                foreach ($return_dates as $dates) { 
                    ...
                    if($dates != ""){
                        ...
                        $sql1 = mysqli_query($con, "SELECT * FROM `fx_earnings` WHERE userid='$userid' AND plan='$plan_name' AND created='$dates' AND plan_id='$planid'");
                        $rows2 = mysqli_num_rows($sql1) ;
                        if($rows2 < 1){//check if earnings have not be returned
                ...
                $sql3 = mysqli_query($con, "SELECT * FROM `fx_total_earned` WHERE userid='$userid' AND plan='$plan_name' AND created='$endingdate' AND plan_id='$planid'");
                $rows4 = mysqli_num_rows($sql3) ;
                if($rows4 < 1){//check if earnings have not be returned
                ...
                //return for ending date
                $sql5 = mysqli_query($con, "SELECT * FROM `fx_total_earned` WHERE userid='$userid' AND plan='$plan_name' AND created='$endingdate' AND plan_id='$planid'");
                $rows5 = mysqli_num_rows($sql5) ;
                if($rows5 < 1){//check if earnings have not be returned
                    if($currentdate >= $endingdate1){ //if date is current
                        $amountearned = ($plan_roi / 100)* $amountinvested;
                        $totalamountearned = ($amountearned*$plan_duration) + $amountinvested;
